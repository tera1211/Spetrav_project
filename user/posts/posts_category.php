...
$get_post_by_category=null;
if(empty($country_id)){
    $get_post_by_category=$post->get_post_by_category($category_id);
}elseif(empty($state_id)){
    $get_post_by_category=$post->get_post_by_country_category($country_id,$category_id);
}elseif(empty($city_id)){
    $get_post_by_category=$post->get_post_by_country_state_category($country_id,$state_id,$category_id);
}else{
    $get_post_by_category=$post->get_post_by_country_state_city_category($country_id,$state_id,$city_id,$category_id);
}
?>


<?php if($get_post_by_category==TRUE):?>
<?php foreach($get_post_by_category as $key=>$values):?>
<div class="card my-4">
